...
import { multiSearch, AVAILABLE } from "./tools.js";
...
async function runTask(chatId, query, forcedTask = null) {
  ...
  const deep = ["Розбір"].includes(task.mode) || ["Добірка"].includes(task.mode);
  const synthPrompt =
    `Зроби результат СТРОГО у цій формі:\n${form}\n\n` +
    (deep ? `${CRITERIA_BLOCK}\n\n${LENSES_BLOCK}\n\n` : "") +
    `Тільки реальні дані з матеріалу. Релевантність > популярність. Кожен пункт — практична користь для маркетингу/креативу.${rulesBlock}\n\nМАТЕРІАЛ:\n${material.slice(0, 28000)}`;
